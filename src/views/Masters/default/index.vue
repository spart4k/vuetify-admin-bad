            ...
            <v-dialog v-model="dialog" persistent :max-width="'55%'">
              ...
              <v-card>
                ...
                <v-card-title v-if="changeStage === 3">
                  <v-btn v-for="(item, index) in editedItem.Courses" :key="item.id" :disabled="coursesStage === index + 1" @click="coursesStage = index + 1" color="#EF8A3E" text>
                    {{item.name}}
                  </v-btn>
                </v-card-title>
                <v-card-title v-if="changeStage === 4">
                  <v-btn v-for="(item, index) in editedItem.Diploms" :key="item.id" :disabled="diplomsStage === index + 1" @click="diplomsStage = index + 1" color="#EF8A3E" text>
                    {{item.title}}
                  </v-btn>
                </v-card-title>
                <v-card-title v-if="changeStage === 5">
                  <v-btn v-for="(item, index) in editedItem.Educations" :key="item.id" :disabled="educationsStage === index + 1" @click="educationsStage = index + 1" color="#EF8A3E" text>
                    {{item.specialization}}
                  </v-btn>
                </v-card-title>
                    ...
                    <v-row cols="12" sm="6" md="4">
                      <v-col cols="12" sm="12" :md="formTitle === 'Изменить' ? '12' : '12'">
                        ...
                        <template v-if="editedItem.Diploms && changeStage === 4">
                            ...
                            ></v-text-field>
                            <v-img v-if="editedItem.Diploms[diplomsStage - 1].Files[0]" :src="editedItem.Diploms[diplomsStage - 1].Files[0].url" />
                        </template>
